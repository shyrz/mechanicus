...
You can enable the companion by adding a `companion` section to your setting configuration file (`~/.config/opencode/mechanicus.json` or `.opencode/mechanicus.json`):

```jsonc
{
  "companion": {
    "enabled": true,
    "binaryPath": "/path/to/mechanicus-companion",
...
```bash
bunx mechanicus install --companion=yes
```
...
```text
$XDG_DATA_HOME/opencode/storage/mechanicus/bin/mechanicus-companion
```

If `XDG_DATA_HOME` is unset, this resolves to:

```text
~/.local/share/opencode/storage/mechanicus/bin/mechanicus-companion
```
