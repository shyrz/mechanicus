---
name: mechanicus
description: Configure and improve mechanicus for the current user. Use when users want to tune agents, models, prompts, custom agents, skills, MCPs, presets, or plugin behavior. Also use when recurring workflow friction suggests a safe config or prompt improvement.
---

# mechanicus Configuration Skill
...
mechanicus is configured through a plugin config file, usually:
...
```text
~/.config/opencode/mechanicus.jsonc
```
...
| `~/.config/opencode/mechanicus.jsonc` | User plugin config with comments/trailing commas; takes precedence over `.json` |
| `~/.config/opencode/mechanicus.json` | User plugin config generated by installer |
| `<project>/.opencode/mechanicus.json` | Project-local plugin overrides checked before user config |
| `~/.config/opencode/mechanicus/{agent}.md` | Full prompt replacement for a built-in agent |
| `~/.config/opencode/mechanicus/{agent}_append.md` | Append-only prompt tuning for a built-in agent |
| `~/.config/opencode/mechanicus/{preset}/{agent}.md` | Preset-specific full prompt replacement |
| `~/.config/opencode/mechanicus/{preset}/{agent}_append.md` | Preset-specific append-only prompt tuning |
...
1. If a `preset` is active, check
   `~/.config/opencode/mechanicus/{preset}/` first.
2. Fall back to `~/.config/opencode/mechanicus/`.
...
- Built-in agent `prompt` and `orchestratorPrompt` fields are **not** supported
  in `mechanicus.json[c]`; use markdown prompt override files instead.
...
```text
~/.config/opencode/mechanicus/orchestrator_append.md
```

or for only the `openai` preset:

```text
~/.config/opencode/mechanicus/openai/orchestrator_append.md
```
...
```text
~/.config/opencode/mechanicus/orchestrator.md
```
...
1. **Inspect current setup**
   - Read the existing mechanicus config file.
...
~/.config/opencode/mechanicus/orchestrator_append.md. Apply it?
```
...
public API changes to it. Apply to your mechanicus config?
```
