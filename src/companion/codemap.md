...
- **Producer (manager.ts)**: `CompanionManager` class
  ...
  - Maintains state in a JSON file at `~/.local/share/opencode/storage/mechanicus/companion-state.json`
...
- Binary path determined by:
  ...
  - Default location: `~/.local/share/opencode/storage/mechanicus/bin/`
...
- `MECHANICUS_COMPANION_SESSION_ID`: Set when spawning companion binary
- `MECHANICUS_COMPANION_DEBUG`: Enables debug mode when config.debug is true
...
- Repository: `shyrz/mechanicus`
- Release tag pattern: `companion-v{version}`
- Artifact names: `mechanicus-companion-v{version}-{target}.{tar.gz|zip}`
