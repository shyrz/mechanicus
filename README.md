<div align="center">
  <a href="https://github.com/shyrz/mechanicus/stargazers">
    <img src="img/v3-beta.webp" alt="mechanicus V3 Beta Release" style="border-radius: 10px;">
  </a>
  <h3>✨ mechanicus ✨</h3>
...
mechanicus is an agent orchestration plugin for OpenCode. It includes a built-in team of specialized agents that can scout a codebase, look up fresh documentation, review architecture, handle UI work, and execute well-scoped implementation tasks under one orchestrator.
...
To make OpenCode Go active during install, run `bunx mechanicus@latest install --preset=opencode-go` or change the default preset name in `~/.config/opencode/mechanicus.json` after installation.
...
2. **Refresh and list the models OpenCode can see**:
   ...
   ```
3. **Open your plugin config** at `~/.config/opencode/mechanicus.json`

4. **Update the models you want for each agent**

> [!TIP]
> It's **recommended** to understand how background orchestration works. The **[Orchestrator prompt](https://github.com/shyrz/mechanicus/blob/master/src/agents/orchestrator.ts#L28)** contains the scheduler rules, specialist routing logic, and thresholds for when work should be assigned to background agents. You can always delegate manually by calling a subagent via: `@agentName <task>`
...
> `bunx mechanicus install --skills=force`. This deliberately replaces
...
| <img src="img/skills/mechanicus.webp" width="120" alt="mechanicus artifact"><br>[`mechanicus`](src/skills/mechanicus/SKILL.md) | Configures and safely improves the plugin setup itself | `orchestrator` | ask to tune your setup |
...
`~/.config/opencode/mechanicus.json`: an explicit list, `"*"` for
...
| **[Skills](docs/skills.md)** | Bundled skills such as `simplify`, `codemap`, `clonedeps`, `deepwork`, `verification-planning`, `reflect`, `worktrees`, and `mechanicus` |
